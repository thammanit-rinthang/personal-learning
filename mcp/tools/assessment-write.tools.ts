...
import { attachQuestionsToAssessment, createAssessment, updateAssessment } from "@/services/assessment.service";
...
export function registerAssessmentWriteTools(server: McpServer, actor: Actor) {
  ...
  server.registerTool("update_assessment", {
    description: "Update a draft assessment or create a revision for published content; never publishes content.",
    inputSchema: { assessmentId: z.string().min(1), courseId: z.string().min(1), slug: z.string().min(1), title: z.string().min(1), description: z.string().nullable().optional(), type: z.nativeEnum(AssessmentType), feedbackMode: z.nativeEnum(FeedbackMode).default(FeedbackMode.AFTER_SUBMIT), passingScore: z.number().int().min(0).max(100).default(70), randomizeOrder: z.boolean().default(false), trigger: z.nativeEnum(AssessmentTrigger).default(AssessmentTrigger.MANUAL), isRequired: z.boolean().default(false), maxAttempts: z.number().int().min(1).nullable().optional(), triggerModuleId: z.string().min(1).nullable().optional(), triggerLessonId: z.string().min(1).nullable().optional(), sections: z.array(z.object({ title: z.string().min(1), instructions: z.string().nullable().optional(), position: z.number().int(), questionCount: z.number().int().nullable().optional(), randomize: z.boolean().default(false) })).optional() },
  }, async ({ assessmentId, ...input }) => {
    try { return jsonResult(await updateAssessment(actor, assessmentId, input)); } catch (error) { return mcpErrorResult(error); }
  });
}
